/**
 * scripts/seed-catalog.ts - Seed the full Odogwu gadget-store catalog
 * =====================================================================
 * Builds a complete, comprehensive gadget-store taxonomy and inventory:
 *
 *   1. Ensures the admin user, creator profile and "Odogwu Laptops" store
 *      exist (find-or-create — safe to run repeatedly, users are preserved).
 *   2. Seeds a two-level CATEGORY tree:
 *        Top-level category (Laptops, Phones, Speakers, Power Banks, …)
 *          └── Brand sub-categories (HP, Dell, Apple, JBL, Zealot, Oraimo, …)
 *      EVERY category gets its full set of brand sub-categories, even the
 *      ones we don't have product images for yet — so the shop is ready to
 *      grow. A shopper clicks a category to see everything in it, then a
 *      sub-category (brand) to narrow it down.
 *   3. Uploads every image in
 *        Odogwu-gadget-Frontend/public/assets/imgs/laptops-pics
 *        Odogwu-gadget-Frontend/public/assets/imgs/gadget-pics
 *      to Cloudinary and inserts a listing for each product, tagged with the
 *      right { category, subCategory } slugs.
 *   4. Recomputes the denormalised listingCount on every category.
 *
 * DESTRUCTIVE: wipes listings, categories, carts, savedproducts, orders,
 * reviews and follows so the catalog is internally consistent. Users, the
 * creator and the store are preserved (find-or-create).
 *
...
// ─── Store / admin constants (kept in sync with seed-odogwu-laptops.ts) ──
...
// ─── Image folders (resolve to the storefront public assets) ────────────
const BASE_IMG_DIR = path.resolve(
  __dirname,
  '../../../Odogwu-gadget-Frontend/public/assets/imgs',
);
const FOLDERS: Record<'laptops' | 'gadget', string> = {
...
const naira = (amount: number) => amount * 100; // NGN -> kobo
...
/** Namespaced sub-category slug, e.g. sub('laptops', 'HP') -> 'laptops-hp' */
const sub = (categorySlug: string, brand: string): string =>
  `${categorySlug}-${slugify(brand)}`;

// ─── Cloudinary uploader (cached to avoid re-uploads on re-runs) ────────
const uploadCache: Record<string, string> = {};

async function uploadImage(
  folder: 'laptops' | 'gadget',
  filename: string,
): Promise<string> {
...
// ═══════════════════════════════════════════════════════════════════════
// CATEGORY TAXONOMY
// Top-level category -> brand sub-categories. Comprehensive on purpose:
// brands with no products yet are still created so the store is ready to
// grow. Icons are RemixIcon classes (rendered as <i className={icon}/> on
// the storefront, matching the existing category chips).
// ═══════════════════════════════════════════════════════════════════════
...
// Convenience slugs for the categories the products reference
const CAT = {
  laptops: 'laptops',
  phones: 'phones',
  speakers: 'speakers',
  powerBanks: slugify('Power Banks'), // 'power-banks'
  earpods: 'earpods',
  headsets: 'headsets',
  networking: 'networking',
  camera: slugify('Camera & Photography'), // 'camera-photography'
  home: slugify('Home & Lifestyle'), // 'home-lifestyle'
};

// ═══════════════════════════════════════════════════════════════════════
// PRODUCT CATALOG
// Each product maps to a { category, subCategory } slug pair and one or
// more image files inside FOLDERS[folder]. Prices in NGN (-> kobo on insert).
// ═══════════════════════════════════════════════════════════════════════
...
type Product = {
  ...
  itemName: string;
  description: string;
  condition: 'brand_new' | 'fairly_used' | 'refurbished';
  priceNaira: number;
  quantity: number;
  tags: string[];
  files: string[];
...
const CATALOG: Product[] = [
  ...
  // ── HP ──
  {
    ...
    description:
      'HP EliteBook x360 1030 G2 convertible business ultrabook. Intel Core i5-7300U (7th Gen), 8GB DDR4 RAM, 256GB NVMe SSD, 13.3" FHD touchscreen (2-in-1 hinge), backlit keyboard, fingerprint reader, Wi-Fi + Bluetooth. Lightweight 1.28 kg aluminium build.\n\nIdeal for: students, freelancers, forex trading, light web development, remote work.',
  ...
  },
  {
    ...
    description:
      'HP EliteBook 820 G4 ultra-portable business laptop. Intel Core i5-7300U (7th Gen), 8GB DDR4 RAM, 256GB SSD, 12.5" HD anti-glare display, backlit keyboard, fingerprint reader, MIL-STD-810G tested chassis. Compact 1.34 kg form factor.',
  ...
  },
  {
    ...
    description:
      'HP EliteBook 830 G5 premium business ultrabook. Intel Core i5-8350U (8th Gen quad-core), 8GB DDR4 RAM, 256GB NVMe SSD, 13.3" FHD IPS display, backlit keyboard, fingerprint + smart card reader, USB-C/Thunderbolt.',
  ...
  },
  {
    ...
    description:
      'HP EliteBook 840 G3 — proven business workhorse. Intel Core i5-6300U (6th Gen), 8GB DDR4 RAM, 256GB SSD, 14" FHD display, backlit keyboard, fingerprint reader. Refurbished, inspected, and tested.',
  ...
  },
  {
    ...
    description:
      'HP EliteBook 840 G4 — premium business 14-inch laptop. Intel Core i7-7600U (7th Gen), 16GB DDR4 RAM, 512GB NVMe SSD, 14" FHD IPS anti-glare display, backlit keyboard, fingerprint reader, vPro security.',
  ...
  },
  {
    ...
    description:
      'HP EliteBook 840 G5 — sleek 14-inch business notebook. Intel Core i5-8250U (8th Gen quad-core), 8GB DDR4 RAM, 256GB NVMe SSD, 14" FHD IPS display, backlit keyboard, fingerprint reader, dual-band Wi-Fi + Bluetooth 5.',
  ...
  },
  {
    ...
    description:
      'HP EliteBook 840 G6 — modern slim business laptop. Intel Core i7-8665U (8th Gen quad-core, vPro), 16GB DDR4 RAM, 512GB NVMe SSD, 14" FHD IPS display, backlit keyboard, fingerprint reader, Sure View privacy screen option.',
  ...
  },
  {
    ...
    description:
      'HP EliteBook 840 G8 — modern thin & light business laptop. Intel Core i5-1135G7 (11th Gen) with Intel Iris Xe graphics, 16GB DDR4 RAM, 512GB NVMe SSD, 14" FHD IPS anti-glare display, backlit keyboard, fingerprint reader, Wi-Fi 6 + Bluetooth 5.2.',
  ...
  },
  {
    ...
    description:
      'HP Pavilion 14 — slim everyday laptop. Intel Core i5 with Intel UHD Graphics, 8GB DDR4 RAM, 512GB SSD, 14" FHD micro-edge display, fast charging, dual speakers tuned by B&O.',
  ...
  },
  {
    ...
    description:
      'HP Pavilion Gaming 15 — entry-level gaming and creator laptop. Intel Core i5 (8th Gen), NVIDIA GeForce GTX dedicated graphics, 8GB DDR4 RAM, 256GB SSD + 1TB HDD, 15.6" FHD display, green-backlit keyboard, dual cooling fans.',
  ...
  // ── Dell ──
  {
    ...
    description:
      'Dell Latitude 3500 — reliable 15.6" business laptop. Intel Core i5-8265U (8th Gen quad-core), 8GB DDR4 RAM, 256GB SSD, 15.6" FHD anti-glare display, full-size keyboard with number pad.',
    condition: 'fairly_used', priceNaira: 295000, quantity: 4,
  ...
  },
  {
    ...
    description:
      'Dell Latitude 7390 — premium 13.3" business ultrabook. Intel Core i7-8650U (8th Gen vPro), 16GB DDR4 RAM, 512GB NVMe SSD, 13.3" FHD IPS display, backlit keyboard, fingerprint reader, carbon-fibre composite chassis, Thunderbolt 3.',
  ...
  },
  {
    ...
    description:
      'Dell Latitude 5289 (2-in-1 convertible). Intel Core i5-7300U (7th Gen), 8GB DDR4 RAM, 256GB SSD, 12.5" FHD touchscreen with 360° hinge, backlit keyboard, active pen support, fingerprint reader.',
  ...
  },
  {
    ...
    description:
      'Dell Latitude E7450 — proven 14" business ultrabook. Intel Core i5-5300U (5th Gen), 8GB DDR3L RAM, 256GB SSD, 14" FHD display, backlit keyboard, fingerprint reader. Inspected and refurbished.',
  ...
  },
  {
    ...
    description:
      'Dell XPS 13 — premium InfinityEdge ultrabook. Intel Core i7, 16GB LPDDR4X RAM, 512GB NVMe SSD, 13.3" FHD+ near-edgeless display, machined aluminium & carbon-fibre build, backlit keyboard, fingerprint power button, Thunderbolt 3.',
  ...
  },
  {
    ...
    description:
      'Dell XPS 15 — flagship 15.6" creator-class laptop. Intel Core i7 with dedicated NVIDIA graphics, 16GB DDR4 RAM, 512GB NVMe SSD, 15.6" FHD InfinityEdge display, carbon-fibre palmrest, backlit keyboard, fingerprint reader, Thunderbolt 3.',
  ...
  },
  {
    ...
    description:
      'Dell Precision 5510 — mobile workstation. Intel Core i7 6th Gen, 16GB DDR4 RAM, 512GB SSD, NVIDIA Quadro M1000M dedicated graphics, 15.6" FHD InfinityEdge display, carbon-fibre palmrest, backlit keyboard.',
  ...
  },
  {
    ...
    description:
      'Dell Latitude 3420 — modern 14" business laptop. Intel Core i5-1135G7 (11th Gen) with Intel Iris Xe graphics, 16GB DDR4 RAM, 512GB NVMe SSD, 14" FHD display, spill-resistant keyboard, MIL-STD-810H tested.',
  ...
  // ── Lenovo ──
  {
    ...
    description:
      'Lenovo ThinkPad X1 Carbon Gen 6 — premium business ultrabook. Intel Core i7-8650U (8th Gen vPro), 16GB LPDDR3 RAM, 512GB NVMe SSD, 14" 2K (2560×1440) IPS display, backlit keyboard, fingerprint reader, carbon-fibre roll cage, Thunderbolt 3. Weight 1.13 kg.',
  ...
  },
  {
    ...
    description:
      'Lenovo Yoga 720 — slim 13.3" 2-in-1 convertible. Intel Core i7 with Intel HD Graphics, 8GB DDR4 RAM, 256GB NVMe SSD, 13.3" FHD IPS touchscreen with 360° hinge, backlit keyboard, fingerprint reader, JBL stereo speakers, Thunderbolt 3.',
  ...
  },
  {
    ...
    description:
      'Lenovo ThinkPad L480 — durable 14" business laptop. Intel Core i5-8250U (8th Gen quad-core), 8GB DDR4 RAM, 256GB SSD, 14" FHD IPS display, spill-resistant ThinkPad keyboard, fingerprint reader, MIL-STD durability testing.',
  ...
  // ── Asus ──
  {
    ...
    description:
      'Asus VivoBook 14 — slim everyday laptop. Intel Core i5-8250U (8th Gen quad-core), 8GB DDR4 RAM, 256GB SSD, 14" FHD NanoEdge display, ergonomic hinge, fingerprint reader, lightweight 1.45 kg.',
    condition: 'fairly_used', priceNaira: 295000, quantity: 4,
  ...
  // ── Apple ──
  {
    ...
    description:
      'MacBook Pro 13" 2019 (Touch Bar + Touch ID). Intel Core i5 (8th Gen), 8GB LPDDR3 RAM, 256GB SSD, 13.3" Retina display, Touch Bar, Touch ID, four Thunderbolt 3 ports, backlit Magic Keyboard. Cycle count ~260.',
  ...
  },
  {
    ...
    description:
      'MacBook Pro 14" with Apple M1 Pro chip (8-core CPU, 14-core GPU), 16GB unified memory, 512GB SSD, 14.2" Liquid Retina XDR display (ProMotion 120Hz), MagSafe charging, Touch ID, three Thunderbolt 4 ports, HDMI, SD card slot. Open-box, cycle count ~230.',
  ...
  },
  {
    ...
    description:
      'MacBook Pro 13" 2020 (Two Thunderbolt 3 ports model). Intel Core i5 1.4 GHz Quad-Core (10th Gen), 8GB LPDDR3 RAM, 256GB SSD, 13.3" Retina display with True Tone, Touch Bar, Touch ID, Magic Keyboard with scissor mechanism.',
  ...
  },
  {
    ...
    description:
      'MacBook Air with Apple M1 chip (8-core CPU, 7-core GPU), 8GB unified memory, 256GB SSD, 13.3" Retina display with True Tone, Touch ID, Magic Keyboard, fanless silent design, up to 18-hour battery.',
  ...
  },
  {
    ...
    description:
      'MacBook Pro 15" with Touch Bar. Intel Core i7 quad-core, 16GB DDR4 RAM, 256GB SSD, Radeon Pro dedicated graphics, 15.4" Retina display, Touch Bar with Touch ID, four Thunderbolt 3 ports, backlit keyboard.',
  ...
  // ═══════════════════════ PHONES ═══════════════════════
  {
    ...
    description:
      'Apple iPhone 12 with A14 Bionic chip, 6.1" Super Retina XDR OLED display, dual 12MP cameras, Face ID, 5G, Ceramic Shield, MagSafe. Clean UK-used unit, battery health 85%+.',
  ...
  },
  {
    ...
    description:
      'Apple iPhone 12 Pro with A14 Bionic, 6.1" Super Retina XDR OLED, triple 12MP Pro camera system with LiDAR, stainless steel frame, Face ID, 5G, MagSafe. Battery health 85%+.',
  ...
  },
  {
    ...
    description:
      'Apple iPhone 14 Pro Max — sealed brand new. A16 Bionic, 6.7" Super Retina XDR OLED with Dynamic Island and Always-On display, 48MP Pro camera system, stainless steel frame, Face ID, 5G. 1-year warranty.',
  ...
  },
  {
    ...
    description:
      'Apple iPhone 15 — sealed brand new. A16 Bionic, 6.1" Super Retina XDR OLED with Dynamic Island, 48MP main camera, USB-C, Ceramic Shield, Face ID, 5G. 1-year warranty.',
  ...
  },
  {
    ...
    description:
      'Samsung Galaxy S21 5G with Exynos 2100 / Snapdragon 888, 6.2" Dynamic AMOLED 2X 120Hz display, triple rear camera, IP68, wireless charging, 5G. Clean UK-used unit.',
  ...
  },
  {
    ...
    description:
      'Google Pixel flagship with stock Android, industry-leading computational photography, OLED display, Titan security, and guaranteed Android updates. Clean UK-used unit.',
  ...
  // ═══════════════════════ SPEAKERS ═══════════════════════
  {
    ...
    description:
      'JBL portable Bluetooth speaker with JBL Pro Sound, deep bass, IPX7 waterproof rating, wireless Bluetooth streaming, and long-lasting battery. Perfect for outdoors, parties and travel.',
  ...
  },
  {
    ...
    description:
      'Zealot S32 compact Bluetooth speaker with rich bass, wireless Bluetooth 5.0, TF card + AUX support, and a rechargeable battery. Great for desk, bedroom, and travel.',
  ...
  },
  {
    ...
    description:
      'Zealot S49 wireless speaker with powerful stereo sound, pulsing RGB light show, Bluetooth 5.0, TWS pairing, FM radio, TF/AUX inputs, and a big rechargeable battery.',
  ...
  },
  {
    ...
    description:
      'Zealot party speaker with high-output drivers, deep thumping bass, TWS dual-pairing, Bluetooth 5.0, and dynamic lighting. Fills a room for hangouts and small events.',
  ...
  },
  {
    ...
    description:
      'Zealot portable wireless speaker with clear highs, solid bass, Bluetooth 5.0, AUX/TF playback, and all-day battery life in a rugged, grab-and-go body.',
  ...
  // ═══════════════════════ POWER BANKS ═══════════════════════
  {
    ...
    description:
      'Oraimo 20000mAh power bank with fast charging, dual USB output + USB-C, LED capacity indicator, and multi-protection safety. Charges phones multiple times over.',
  ...
  },
  {
    ...
    description:
      'Slim 10000mAh power bank with dual output, USB-C input, and LED indicator. Lightweight and pocket-friendly for everyday top-ups.',
  ...
  },
  {
    ...
    description:
      'High-capacity 20000mAh power bank with dual USB outputs, fast charging support, and a digital battery display. Keeps phones and tablets going all day.',
  ...
  },
  {
    ...
    description:
      'Fast-charge power bank with Power Delivery (PD) and Quick Charge (QC) support, USB-C in/out, and a compact design that refuels modern phones rapidly.',
  ...
  },
  {
    ...
    description:
      'Ultra-slim travel power bank with a premium finish, dual output, and reliable capacity for on-the-go charging.',
  ...
  },
  {
    ...
    description:
      'Rugged solar power bank with a built-in panel for emergency top-ups, dual USB outputs, LED torch, and a water-resistant body. Ideal for outdoors and travel.',
  ...
  // ═══════════════════════ EARPODS ═══════════════════════
  {
    ...
    description:
      'True wireless stereo (TWS) earbuds with Bluetooth 5.0, touch controls, clear calls, and a compact charging case for all-day listening.',
  ...
  },
  {
    ...
    description:
      'Pro wireless earbuds with environmental noise cancellation (ENC) for calls, punchy bass, touch controls, and long battery life with the charging case.',
  ...
  },
  {
    ...
    description:
      'Sony wireless earbuds with signature Sony sound, strong noise isolation, clear hands-free calls, touch controls, and a compact charging case.',
  ...
  // ═══════════════════════ HEADSETS ═══════════════════════
  {
    ...
    description:
      'Comfortable over-ear wireless headset with Bluetooth, built-in microphone, soft ear cushions, and long battery life. Great for calls, music and study.',
  ...
  },
  {
    ...
    description:
      'Foldable over-ear headphones with deep bass, cushioned headband, wired + wireless modes, and an inline mic. Comfortable for long sessions.',
  ...
  // ═══════════════════════ NETWORKING ═══════════════════════
  {
    ...
    description:
      'Starlink standard kit for high-speed, low-latency satellite internet anywhere. Includes dish, Wi-Fi router, cables and mount. Perfect for homes and businesses beyond fibre coverage.',
  ...
  // ═══════════════════════ HOME & LIFESTYLE ═══════════════════════
  {
    ...
    description:
      'Itel rechargeable fan with a strong quiet motor, long battery runtime, adjustable speeds, and built-in LED light. Keeps you cool through power outages.',
  ...
  },
  {
    ...
    description:
      'Rechargeable standing fan with multiple speed settings, wide oscillation, USB charging, and a long-lasting battery for cool air anytime.',
  ...
  },
  {
    ...
    description:
      'Itel inverter for reliable home backup power — clean, stable output for lights, fans, TVs and small appliances during outages. Efficient and easy to set up.',
  ...
  },
  {
    ...
    description:
      'Bright rechargeable LED light with multiple brightness modes, long runtime, and a hanging hook. Perfect emergency and outdoor lighting for power cuts.',
  ...
  // ═══════════════════════ CAMERA & PHOTOGRAPHY ═══════════════════════
  {
    ...
    description:
      'Professional tripod stand with adjustable height, 360° pan head, quick-release plate, and a sturdy build for cameras, phones and ring lights.',
  ...
  },
  {
    ...
    description:
      'Lightweight travel camera tripod with foldable legs, adjustable head, and a compact carry size. Great for vlogging, photography and live streams.',
  ...
  },
  {
    ...
    description:
      'Versatile phone tripod stand with a secure phone mount, adjustable height and angles. Ideal for content creation, video calls and photos.',
  ...
  },
  {
    ...
    description:
      'Extendable selfie stick with a Bluetooth shutter remote, secure phone clamp, and a foldable pocket-size design for photos and vlogging on the go.',
...
async function seed() {
  ...
  // ── 1. Ensure admin user (preserve if exists)
  ...
  // ── 2. Ensure creator profile (preserve if exists)
  ...
  // ── 3. Ensure store (preserve if exists)
  ...
  if (!store) {
    ...
    await db.collection('stores').insertOne({
      ...
      logo: STORE_LOGO,
      description:
        "Nigeria's trusted gadget store — laptops, phones, speakers, power banks, and accessories from the brands you love.",
  ...
  // ── 4. Wipe catalog collections (keep users/creator/store)
  for (const col of [
    'listings',
    'categories',
    'savedproducts',
  ...
  // ── 5. Seed category taxonomy (top-level + brand sub-categories)
  const parentIdBySlug: Record<string, mongoose.Types.ObjectId> = {};
  ...
  console.log(
    `✅ Created ${parentCount} categories + ${categoryDocs.length - parentCount} sub-categories\n`,
  );

  // ── 6. Upload images + build listing docs
  const totalImages = CATALOG.reduce((s, p) => s + p.files.length, 0);
  console.log(
    `📦 Seeding ${CATALOG.length} products with ${totalImages} images...\n`,
  );
  ...
  // ── 7. Recompute denormalised counters
  ...
  await db
    ...
    .updateOne(
      { _id: store._id },
      { $set: { totalListings: listingDocs.length, updatedAt: now } },
    );
  await db
    .collection('creators')
    .updateOne(
