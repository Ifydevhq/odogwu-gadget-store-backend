/**
 * scripts/seed-real-laptops.ts - Seed real Odogwu Laptops inventory
 * ===================================================================
 * Uploads every image in Odogwu-Laptops-Frontend/public/assets/imgs/laptops-pics
 * to Cloudinary, wipes the existing listings (plus carts/saved/orders/reviews
 * that reference them), then inserts a fresh product catalog under the
 * existing Odogwu Laptops store (preserves the seeded logo + creator).
 *
 * Run: yarn seed:laptops
 *
 * Requires .env:
 *   MONGODB_URI=...
...
const IMAGES_DIR = path.resolve(
  __dirname,
  '../../../Odogwu-Laptops-Frontend/public/assets/imgs/laptops-pics',
);
...
const naira = (amount: number) => amount * 100; // Convert NGN to kobo

// ─── Cloudinary uploader (with caching to avoid re-uploads on re-runs) ─
...
// ─── Product catalogue ──────────────────────────────────────────────────
// Prices are NGN (will be converted to kobo on insert). All laptops under
// the Odogwu Laptops single-store. Conditions: brand_new | fairly_used
// (a.k.a. UK used) | refurbished.
...
type Product = {
  ...
  quantity: number;
  tags: string[];
  files: string[]; // image filenames (in laptops-pics)
};
...
async function seed() {
  ...
  // ── Find existing store + creator (preserve, don't recreate)
  ...
  // ── Wipe everything that references listings
  ...
  // ── Update category listingCount values
  ...
  await db
    .collection('categories')
    .updateMany(
      { name: { $nin: Object.keys(byCategory) } },
      { $set: { listingCount: 0, updatedAt: new Date() } },
    );
